...
checksum and leaves it in `~/.local/bin`. When that directory is not on your
PATH, the line that puts it there is added to your shell's startup file, so a
new shell finds `cupstui` — `CUPSTUI_NO_MODIFY_PATH` leaves your files alone and
prints the line instead. `CUPSTUI_INSTALL_DIR` puts the binary somewhere else,
`CUPSTUI_VERSION` pins a tag.
